...
for youbike in youbikes:
    ...
    locat = haversine(121.268160, 24.970612, lng, lat)
    if int(youbike.get('sbi')) >= sbi and int(youbike.get('bemp')) >= bemp and locat <= m:
        print(youbike.get('sna'), youbike.get('sbi'), youbike.get('bemp'), locat, 'm')